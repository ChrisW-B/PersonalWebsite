...
const webpackManifest = process.env.NODE_ENV === `production` ? require(`../../public/build/client/manifest.json`) : {};
...
const hasVendors = webpackManifest && Object.prototype.hasOwnProperty.call(webpackManifest, `vendors.js`);
const hasRuntime = webpackManifest && Object.prototype.hasOwnProperty.call(webpackManifest, `runtime.js`);
const hasApp = webpackManifest && Object.prototype.hasOwnProperty.call(webpackManifest, `app.js`);

if (!hasVendors || !hasRuntime || !hasApp) {
  console.warn(`missing build`);
}

const vendorsRoot = hasVendors ? webpackManifest[`vendors.js`] : ``;
const vendorsPath = vendorsRoot.includes(`build/client`) ? vendorsRoot : path.join(`/build`, `client`, vendorsRoot);

const runtimeRoot = hasRuntime ? webpackManifest[`runtime.js`] : ``;
const runtimePath = runtimeRoot.includes(`build/client`) ? runtimeRoot : path.join(`/build`, `client`, runtimeRoot);

const appRoot = hasApp ? webpackManifest[`app.js`] : ``;
