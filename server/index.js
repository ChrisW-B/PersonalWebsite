...
app.get('/github', async(req, res) => {
  ...
  try {
    ...
    const commits = refs.map((ref) => {
      const { messageHeadlineHTML, messageBodyHTML, committedDate } = ref.target.history.edges[0].node
      return {
        success: true,
    ...
    res.send({ ...mostRecentCommit, time: relativeTimeDifference(new Date(mostRecentCommit.time)) });
  } catch (e) {
